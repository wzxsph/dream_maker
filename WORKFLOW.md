...
**造梦**是一个 H5 互动短剧生成器。用户输入一句"脑洞"，AI 生成一段有分支的3幕互动短剧，包含选择点、视觉主题和 UI 特效。玩家在每个续写点继续当前剧情，体验已预生成好的下一幕。

- **前端**：原生 HTML + CSS + JavaScript
- **后端**：Node.js + Express
- **存储**：本地 JSON 文件（`backend/stories/{story_id}.json`）
- **AI**：MiniMax Anthropic 兼容 API（未配置 key 时自动走 mock 兜底）
- **叙事架构**：所有生成内容服从 [微场景锁结局三幕架构](./STORY_ARCHITECTURE.md)
...
- **微场景**：一个核心场景，最多一个相邻小空间，故事时间 10-30 分钟。
- **小演员表**：最多 3 个有姓名角色，避免生成时发散。
- **伪开放式**：分支改变手段、代价、谁先暴露，但每幕内汇聚。
- **锁结局**：开场写入 `story_state.architecture.ending_lane`，只能是 `truth_reversal`、`price_escape`、`role_swap` 之一。
- **固定节点图**：三幕分别使用 `node_0/1_a/1_b/2`、`node_5/6_a/6_b/7`、`node_8/9_a/9_b/10_ending`。

代码落点：`backend/config/storyArchitecture.js` 定义蓝图，所有 prompt 和 `graphValidator` 都引用或校验这套结构。
...
### 前端流程
1. 校验非空（最多200字）
2. 显示全屏加载遮罩，文字为 **"正在构思故事..."**
3. 调用 `POST /api/stories`，请求体 `{ prompt }`
4. 收到响应 `{ story_id, title, synopsis, status }`
5. 展示简介页，URL hash 变为 `#/story/{storyId}`
6. 后台生成第一幕，前端轮询故事状态
7. 生成完成后从 `chunk.nodes` 构建 `nodesMap`
8. `renderNode()` 渲染起始节点的文字和选项按钮

### 后端 Pipeline（`initialStoryPipeline`）
1. **内容安全校验** — `moderationService.validateUserPrompt()` 检查敏感词
2. **生成故事ID** — `nanoid`
3. **Layer 0** — `buildStoryIntroPrompt()` 生成 `title` + `synopsis`
4. **创建会话** — `storySessionService.createSession()` 写入默认 `story_state.architecture`
5. **后台任务** — `generationJobService.startGenerationJob()` 执行 `generateOpeningChunk`
6. **Layer 1** — `buildStoryContentPrompt()` 生成：
   - `story_state`：流派、基调、角色、事实、悬念、架构字段
   - `state_patch`：初始状态变更
   - `content.fragments`：4 个纯文本片段
7. **Layer 2** — `contentToJsonService.fragmentsToChunkJson()` 转为第一幕节点图
8. **规范化** — `storyNormalizer.normalizeStoryResult()`
   - 修正主题名称
   - 去重并限制选项数量
   - 清除无效的 UI 特效
9. **结构校验** — `storyValidator.validateStoryResult()` + `graphValidator.validateChunkGraph()`
   - 确认 `start_node` 存在
   - 确认所有 `end_nodes` 可达
   - 确认节点 ID 严格符合当前幕蓝图
   - 确认非最终幕包含可达的 `__GENERATE_NEXT__`
10. **内容审核** — `moderationService.moderateChunk()` 拒绝违禁词
11. **状态合并** — `storyStateService.mergeStatePatch()` 更新幕数/事实/悬念/角色
12. **构建卡片** — `storyCardService.buildStoryCards()` 创建上下文卡片：
   - `idea_seed`：原始脑洞
   - `premise`：故事卖点
   - `protagonist`：主角卡
   - `character`：角色卡
   - `architecture`：微场景、伪开放、结局走向
   - `style_guide`：短剧写作指南
13. **存储** — `storageService.saveStorySession()` 写入 `backend/stories/{storyId}.json`
14. **状态置为 ready**

### AI Prompt（初始生成）
初始内容 prompt 指示 AI 输出：
- `title`：吸引人的故事标题
- `story_state.architecture`：`scene_lock`、`choice_contract`、`ending_lane`、`ending_promise`
- `state_patch`：初始状态变更
- `content.fragments`：第一幕 4 个纯文本片段
...
### 用户选项
当选择导致 `__GENERATE_NEXT__` 时，弹出面板提供两个按钮：
...
```json
{
  ...
  "story_state": {
    ...
    "architecture": {
      ...
      "ending_lane": "truth_reversal",
      "ending_promise": "关键证据公开，主角夺回主动权"
    }
  },
  "chunks": [
    { "chunk_id": "chunk_1", "chunk_index": 1, "type": "opening", "nodes": {...} },
    { "chunk_id": "chunk_2", "chunk_index": 2, "type": "middle", "nodes": {...} },
    { "chunk_id": "chunk_3", "chunk_index": 3, "type": "ending", "nodes": {...} }
  ],
  "cards": [
    { "type": "idea_seed", "content": {} },
    { "type": "premise", "content": {} },
    { "type": "architecture", "content": {} },
    { "type": "style_guide", "content": {} }
  ],
  "node_index": {
    "node_0": { "chunk_id": "chunk_1", "chunk_index": 1 },
    "node_1_a": { "chunk_id": "chunk_1", "chunk_index": 1 }
  },
  "player_path": [
    { "current_node_id": "node_0", "choice_content": "推开那扇门", "created_at": "..." },
    { "current_node_id": "node_2", "choice_content": "抓住监控红点继续追击", "created_at": "..." }
  ],
  "interventions": [
...
| `POST` | `/api/stories` | 创建新故事 | `{ story_id, title, synopsis, status }` |
...
backend/
  server.js
  config/
    storyArchitecture.js           — 微场景锁结局三幕架构
  services/
...
- **第一幕**：`node_0 → node_1_a / node_1_b → node_2 → __GENERATE_NEXT__`
...
`story_state.architecture.ending_lane` 在开场确定，后续续写和玩家改写都只能沿着该走向推进。A/B 分支改变的是手段、代价、信息揭露顺序，而不是另开一条无限剧情树。
